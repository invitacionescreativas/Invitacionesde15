...
function updateTimer() {
  const targetDate = new Date("december 04, 2026 22:00:00").getTime();
...
// --------------------------- Google Sheets ---------------------------
const GOOGLE_SHEETS_URL = "https://script.google.com/macros/s/AKfycbwauyH8bpRvjUGtI5MUuKRi4xWRitUpjmYxD9noa6A-LqgVCIAwbXBUUtL2tX0ke7v5/exec";

// Apps Script no habilita CORS para leer la respuesta desde GitHub Pages.
// no-cors envía la solicitud, pero NO permite comprobar que la fila se guardó.
async function enviarASheets(datos, boton) {
  if (boton.disabled) return false;
  boton.disabled = true;
  ...
  try {
    ...
    // Respuesta opaca: no afirmar que Google Sheets confirmó el guardado.
    alert('Su respuesta fue enviada con éxito');
    return true;
  } catch (error) {
    console.error('Error al enviar el formulario:', error);
    alert('No se pudo enviar. Revisá tu conexión e intentá nuevamente.');
    return false;
  } finally {
...
document.addEventListener('DOMContentLoaded', () => {
  const boton = document.getElementById('botonplay1');
  if (!boton) return;
  boton.addEventListener('click', async () => {
    ...
    const enviado = await enviarASheets({ tipo: 'Canciones', nombre, cancion }, boton);
    if (enviado) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  boton.addEventListener('click', async () => {
    ...
    const opcion = document.querySelector('input[name="attendanceOption"]:checked');
    if (!opcion) { alert('Por favor, seleccioná si asistirás o no.'); return; }
    if (!nombre) { alert('Por favor, escribí tu nombre y apellido.'); return; }
    const enviado = await enviarASheets({
      tipo: 'Asistencia', nombre, asistencia: opcion.value, mensaje
    }, boton);
    if (enviado) {
...
document.addEventListener('DOMContentLoaded', () => {
  const boton = document.getElementById('botonRestriccion1');
  if (!boton) return;
  boton.addEventListener('click', async () => {
    const nombre = document.getElementById('foodFullName').value.trim();
    const restriccion = document.getElementById('foodRestriction').value.trim();
    if (!nombre) { alert('Por favor, escribí tu nombre y apellido.'); return; }
    if (!restriccion) { alert('Por favor, detallá tu restricción o preferencia alimentaria.'); return; }
    const enviado = await enviarASheets({ tipo: 'Restricciones', nombre, restriccion }, boton);
    if (enviado) {
      document.getElementById('foodFullName').value = '';
      document.getElementById('foodRestriction').value = '';
    }
  });
});
