...
function updateTimer() {
  const targetDate = new Date("march 07, 2026 21:30:00").getTime();
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function sendMessage() {
    ...
    if (selectedContact === '') {
      alert('Por favor, elegí a quién querés enviar el mensaje.');
      return;
    }

    const finalMessage = `*Presencia:* ${attendanceStatus.value}\n*Nombre y Apellido:* ${userName}\n*Mensaje:* ${userMessage ? userMessage : 'N/A'}`;
    const whatsappLink = `https://wa.me/${selectedContact}?text=${encodeURIComponent(finalMessage)}`;
...
// ---------------------------------------------------------------
// FORMULARIO RESTRICCIONES ALIMENTARIAS (no lo tocamos salvo números)
// ---------------------------------------------------------------
document.addEventListener('DOMContentLoaded', function () {

    function sendFoodMessage() {
        const userName = document.getElementById('foodFullName').value.trim();
        const restrictionInfo = document.getElementById('foodRestriction').value.trim();
        const selectedContact = document.getElementById('contactSelectFood').value;
        ...
        if (restrictionInfo === '') {
            alert('Por favor, detallá tu restricción o preferencia alimentaria.');
            return;
        }
        ...
        const finalMessage = `*Restricciones Alimentarias*\n*Nombre:* ${userName}\n*Detalle:* ${restrictionInfo}`;
        const whatsappLink = `https://wa.me/${selectedContact}?text=${encodeURIComponent(finalMessage)}`;
        ...
        document.getElementById('contactSelectFood').value = '';

        document.getElementById('restricciones').scrollIntoView({ behavior: 'smooth' });
    }

    // Botón único para restricciones
    document.getElementById('botonRestriccion1').addEventListener('click', sendFoodMessage);
