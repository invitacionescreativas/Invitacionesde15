...
function updateTimer() {
  const targetDate = new Date("january 10, 2026 22:00:00").getTime();
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function sendMessage() {
    ...
    const phoneNumber = document.getElementById('destinatario').value;

    if (name.trim() === '' || message.trim() === '' || phoneNumber === '') {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function sendFoodCondition() {
    const name = document.getElementById('foodName').value;
    const condition = document.getElementById('foodCondition').value;
    const extra = document.getElementById('foodExtra').value;
    const phone = document.getElementById('foodRecipient').value;

    if (name.trim() === '' || condition === '' || phone === '') {
      alert('Por favor, completa los campos obligatorios.');
      return;
    }

    let finalCondition = condition;
    if (condition === 'Otra' && extra.trim() !== '') {
      finalCondition = extra;
    } else if (extra.trim() !== '') {
      finalCondition += ` — ${extra}`;
    }

    const fullMessage = `*Condición alimentaria*\n\n*Nombre:* ${name}\n*Condición:* ${finalCondition}`;
    const whatsappURL = `https://wa.me/${phone}?text=${encodeURIComponent(fullMessage)}`;

    window.open(whatsappURL, '_blank');

    alert('Condición enviada');

    document.getElementById('foodName').value = '';
    document.getElementById('foodCondition').value = '';
    document.getElementById('foodExtra').value = '';
    document.getElementById('foodRecipient').value = '';

    document.querySelector('#alimentacion').scrollIntoView({ behavior: 'smooth' });
  }

  document.getElementById('botonFood').addEventListener('click', sendFoodCondition);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function sendMessage() {
    ...
    const phoneNumber = document.getElementById('attendanceRecipient').value;
    ...
    document.getElementById('attendanceRecipient').value = '';
