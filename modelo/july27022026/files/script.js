...
function updateTimer() {
  const targetDate = new Date("february 27, 2026 21:30:00").getTime();
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const phoneNumber1 = '543546454206'; // Número para el primer botón
  const phoneNumber2 = '543546454206'; // Número para el segundo botón
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const phones = {
    diego: '541139280532',
    alejandra: '541158148607'
  };

  document.getElementById('botonMenu').addEventListener('click', function () {

    const name = document.getElementById('foodName').value;
    const restriction = document.getElementById('foodRestriction').value;
    const sendTo = document.getElementById('sendTo').value;

    if (name.trim() === '' || restriction === '' || sendTo === '') {
      alert('Por favor completa todos los campos 😊');
      return;
    }

    const message = `Hola 😊\n\n*Nombre:* ${name}\n*Restricción alimentaria:* ${restriction}`;
    const phoneNumber = phones[sendTo];
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  btn.addEventListener('click', function () {
    ...
    const sendToSelect = document.getElementById('sendToConfirmacion');
    ...
    if (userNameInput.value.trim() === '' || sendToSelect.value === '') {
      alert('Por favor, completa tu nombre y elegí a quién enviarlo.');
      return;
    }

    const finalMessage = `Hola 😊\n\n*Presencia:* ${attendanceStatus.value}\n*Nombre y Apellido:* ${userNameInput.value.trim()}\n*Mensaje:* ${messageInput.value.trim() || '—'}`;

    const phoneNumber = phones[sendToSelect.value];
