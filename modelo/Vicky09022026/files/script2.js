...
function updateTimer() {
  const targetDate = new Date("february 10, 2026 00:30:00").getTime();
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const phoneNumber1 = '542634507456'; // Número para el primer botón
  const phoneNumber2 = '542634507456'; // Número para el segundo botón
  ...
  function sendMessage(phoneNumber) {
    ...
    const fullMessage = `🎵 *Tema recomendado:* ${message}`;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function sendMessage() {
    ...
    const recipientKey = document.getElementById('destinatario').value;

    if (!recipientKey) {
      alert('Por favor seleccioná a quién enviar el mensaje.');
      return;
    }
    ...
    const phoneNumber = numbers[recipientKey];
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const menuNumber = '542634507456'; // mismo número o podés cambiarlo

  function sendMenuRestriction() {
    const name = document.getElementById('menuFullName').value.trim();
    const restriction = document.getElementById('menuRestriction').value.trim();
    ...
    if (restriction === '') {
      alert('Por favor indicá tu restricción alimentaria');
      return;
    }

    const finalMessage = `*Restricción alimentaria*\n*Nombre:* ${name}\n*Detalle:* ${restriction}`;
    const whatsappLink = `https://wa.me/${menuNumber}?text=${encodeURIComponent(finalMessage)}`;

    window.open(whatsappLink, '_blank');

    alert('Restricción enviada. ¡Gracias!');

    document.getElementById('menuFullName').value = '';
    document.getElementById('menuRestriction').value = '';

    document.getElementById('menu').scrollIntoView({ behavior: 'smooth' });
  }

  document.getElementById('botonMenu').addEventListener('click', sendMenuRestriction);
