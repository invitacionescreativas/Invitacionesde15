...
function updateTimer() {
  const targetDate = new Date("march 14, 2026 21:00:00").getTime();
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('botonMenu').addEventListener('click', function () {
    ...
    if (name.trim() === '' || restriction.trim() === '') {
      alert('Por favor completá nombre y restricción.');
      return;
    }
    ...
    const whatsappURL = `https://wa.me/${menuPhone}?text=${encodeURIComponent(fullMessage)}`;
    ...
    document.getElementById('menuRestriccion').scrollIntoView({ behavior: 'smooth' });
  });
});
// --------------------------------Invitados---------------------------------
...
document.addEventListener('DOMContentLoaded', function () {
  // Definir los números de teléfono
  const recipientNumber1 = '543546454206'; // Número para el primer botón
  const recipientNumber2 = '543546454206'; // Número para el segundo botón
